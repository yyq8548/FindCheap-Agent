...
4. Search Awin, Shopify, eBay in parallel. For a required brand with a verified storefront, the tool also searches the official Shopify store. Never run passes manually. Preserve returned order and the three presentation groups: official website matches (maximum 2), trusted exact/similar matches, then best-value high-match options. Hide diagnostics. Commercial relationships never change relevance ranking.
