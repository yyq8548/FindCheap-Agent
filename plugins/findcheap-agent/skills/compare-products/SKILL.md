...
# FindCheap Agent v0.6.13 Shopify Global Catalog
...
2. **Selected-product quote identity is immutable.** Every returned product includes `quoteReference` with the search `renderId` and exact Shopify `variantId`. When the user later selects one of those products and supplies a ZIP, call `quote_selected_shopify_product` exactly once with `{ renderId: quoteReference.renderId, variantId: quoteReference.variantId, zipCode }`. Never call `search_shopify_products` again by title, never convert the title into a handle, and never substitute another variant. If the reference expired or the merchant cannot quote it, report that exact failure; do not search a replacement. This is the only supported way to claim that item price, shipping, tax, and estimated total belong to the same returned variant.
