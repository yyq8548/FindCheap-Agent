...
The Commerce API is the authenticated boundary for audited Commerce reads. The repository currently
has **0 enabled comparison merchants**. Do not turn a merchant on until its separate business,
...
With zero gate-approved merchants and no database, the executable exits without listening. Gate
validation errors fail startup. Production also fails startup without the database URL and token.
