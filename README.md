...
Current source package: **v0.18.8** — hard requirements now use one shared evidence path across Shopify, Awin and WooCommerce. Bounded negation, AND/OR, multiple typed quantities and selected-variant evidence prevent a conflicting product or parent option list from entering the primary recommendation. Search, recovery, inspection and comparison reuse the same result facts; retained research cards are explicitly distinguished from excluded or verified products. WooCommerce still has 1,003 technical search storefronts, and registration does not grant merchant trust. Publication, deployment, installation and native acceptance are verified separately in the release record.
Nine-row comparisons and immutable selections remain.
[v0.18.8 release and verification status](docs/releases/v0.18.8.md). [Requirement-evidence remediation](docs/engineering/changes/2026-09-14-requirement-evidence-remediation.md). [9/14 repair and acceptance status](<改进计划 9_14.md>). [v0.18.7 completion report](docs/releases/v0.18.7-completion-report.md). [1,000-store baseline evidence](docs/engineering/changes/2026-09-08-woocommerce-expansion-1000-data.md). [Existing 200-merchant trust review](docs/product/woocommerce-merchant-trust-200.md).
...
The v0.18.7 category rule uses explicit, recognized product types before query keywords when planning merchants. Known compatible categories receive priority; clearly conflicting categories are excluded. Missing, broad, unsupported or neighboring-category metadata remains uncertain and can use at most two exploration slots per pass. An unknown request type retains the existing bounded planner. The rule changes merchant selection, not product identity or trust, and does not exclude a registered direct-product URL from its existing safety checks.
