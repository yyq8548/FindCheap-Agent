...
- an authorized Chrome skill for a read-only, web-wide v0.2.2 fallback search;
...
user-authorized Chrome v0.2.2 fallback path remains available.
...
v0.2.2 classifies Shopify candidates as `EXACT`, `SIMILAR`, or internal `IRRELEVANT`. Irrelevant
...
The current registry contains ten technically verified pilots and accepts at most fifty checked-in
entries. Per-store failures and timeouts are isolated and reported through coverage diagnostics.
