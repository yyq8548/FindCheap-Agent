...
用户附上商品图片或截图后，Codex 只提取画面中可观察到的商品类型、品牌或 Logo 文字、型号/款号线索、颜色、材质、图案、版型和长度。随后仍由同一个 `search_products` 路由搜索合格来源，并把结果明确分为三组：`POSSIBLE_SAME_ITEM`（可能同款）、`HIGHLY_SIMILAR`（高度相似）和 `SAME_STYLE`（同风格）。

视觉相似不等于精确身份。除非稳定型号、款号、SKU 或 GTIN 能独立确认身份，否则视觉结果只能保持为 `DISCOVERY_MATCH` 或 `SIMILAR`，不会伪装成 `EXACT`。图片缺少一个影响决策的关键信息时，Agent 最多只追问一次商品类型、尺寸、预算、颜色或使用场景。v0.12 的匹配器包含 30 个视觉属性 Golden Tasks，并设置了不低于 80% 的分组门槛；真实截图的人工 Top-3 相关性仍属于发布评测指标，不会被当作自动身份确认。
