...
用户附上商品图片或截图后，Codex 会把直接观察与低置信度推断分开记录。可提取商品类型、品牌或 Logo、型号/款号线索、颜色、材质、图案、版型、长度、领型、袖型、闭合方式、衣领、腰线、下摆、印花细节、可见文字、独特结构、图片质量和遮挡。交互式图片搜索分两步：`search_visual_candidates` 最多返回 6 张安全加载的候选图，当前 Codex 模型把它们与用户图片逐一比较，再把结构化证据提交给 `finalize_visual_search`。文字搜索仍只调用一次 `search_products`；Watch 和批量任务不执行图片重排。
