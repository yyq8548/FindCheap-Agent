...
默认插件会显示统一商品搜索、商品卡片、Deal Concierge 和 Watch 工具。只有配置完整的服务地址和 Token 后，Commerce 比价和单独的已验证 Deals 工具才会出现。

## 购买决策助手

用户选择商品卡片后，购买决策助手会继续使用该商品和变体的稳定身份，检查当前商品价、库存、已验证商家优惠候选、可用的 ZIP 到手价证据，以及有界的价格历史样本。它不会重新搜索已选商品标题。

结果会给出 `BUY_NOW`、`WAIT` 或 `WATCH`，并同时说明置信度、理由、样本数量、样本日期范围和限制。只有至少取得跨五个不同日期的五条观察记录，才会声称历史低价或典型价格区间；只有观察到至少三次促销事件，才会报告历史促销间隔。插件不会虚构未来降价概率。商家促销在商家确认该商品适用范围和叠加规则前，只作为优惠候选。

配置受保护的价格历史服务后，每次真实查询得到的当前商品价或到手价都会按同一稳定商品身份追加记录。ZIP 和会员上下文只保存为不可逆哈希。新商品在样本不足时仍会安全返回 `WATCH / LOW`，达到跨日证据门槛后才会给出历史低价或促销间隔结论。

`WATCH` 建议不等于创建监控。只有用户明确要求提醒或持续监控后，Codex 才会建立定时 Watch。
