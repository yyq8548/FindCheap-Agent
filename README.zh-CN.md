...
当前源码版本：**v0.18.3**。WooCommerce 访问表扩至 1,000 家，修复胶囊与机器兼容性核验、分页和受限来源恢复，保留九行比较及原选择。部署与已安装插件状态独立核验。[发布范围与验证状态](docs/releases/v0.18.3.md)；[千店数据与证据](docs/engineering/changes/2026-09-08-woocommerce-expansion-1000-data.md)；[可重复验收测试](docs/testing/capsule-search-and-woocommerce-expansion.md)。
...
后续报价时，可以用结果序号或直接选择商品卡片。FindCheap Agent 会复用首次结果中的商品和变体，不会重新按标题搜索。稳定选择在当前 MCP 进程中最多保留两小时；缓存最多保存最近 128 份搜索快照，过期后会要求重新搜索。
