...
Runtime `1c4930cf1464f32e7bf45099097ff46f5081a308` is committed/pushed and tagged v0.18.4. Linux/Windows plugin CI and installer CI passed. Railway deployment `26a940f5-413e-4f47-99c2-59004bae4ee3` succeeded; running source hash equals local, health/readiness passed, every existing official/trust record unchanged. New cache is installed/enabled, all14 plugin files match source, raw bundle hash matches, and installed stdio5/5 passed. The old occupied cache was retained after its backup returned Access denied.

Installed real-upstream SDK coffee/capsule/Original dialogue and immutable rendering passed with actual source failures retained. Release installer assets passed their tag workflow and three downloaded SHA-256 checks. A separate actual metadata-reader check returned ACTIVE for the isolated live QA task and UNKNOWN for the deleted one (62/73ms; not full-turn performance). See [release results](../../releases/v0.18.4.md) and [machine-readable receipt](2026-09-09-v0.18.4-delivery.json). Documentation-only follow-up records delivery without changing deployed or installed runtime hashes. The explicitly listed host/business gates remain open; this delivery does not mark all16 acceptance items passed.
