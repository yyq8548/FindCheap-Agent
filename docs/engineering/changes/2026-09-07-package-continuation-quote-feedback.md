...
Delivery 18:01 UTC: runtime `474dcdb` pushed; both GitHub workflows succeeded. Railway production deployment `ed5a146f-2476-4c76-97c8-6ba02ae345a4` succeeded, deployed hash matches local, health/ready/search HTTP 200. Installed enabled v0.17.31 package: 13 source files equal, bundle hash equal, stdio 4/4. Supported marketplace upgrade activated the new cache despite a subsequent redundant add's locked-cache backup error; no forced overwrite. Details and exact hashes in [release evidence](../../releases/v0.17.31.md). Actual desktop retest and blocked merchant inspection remain explicit, not closed by CI.
