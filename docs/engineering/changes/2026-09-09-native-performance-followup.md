...
### 图片收集器 I3.1：真实候选图阶段（修订前冻结）

I01-R1实际执行后发现原指标只区分首次`products`商品结果、最终回复及总墙钟，缺少视觉检索自身返回候选图的阶段指标。主任务要求保留这些旧指标和原报告，新增`firstVisualCandidateResultMs`：只采用原始、成功完成的`search_visual_candidates`回执，同时有至少1个结构化candidate及至少1个真实image内容块；取最早完成时刻距本回合task_started的毫秒数。空候选、只有描述无图、失败调用、开场白或最终商品卡不能替代此阶段。

新增`visualCandidateResultWithin180s`及全部8位置／6计划执行位置各自的计数和分母。它只表示真实候选图已供模型复核，不表示同款已确认、已有最终商品卡或可购买。总墙钟与最终回复的180秒指标不改变；等待继续UNKNOWN，不从模型或用户聊天时间扣除。

仅改ignored图片收集器、固定时间自测和输出schemaVersion2；I01原`native-image-performance.json`保留，新口径输出另存`native-image-performance-v2.json`。先验证真实候选图、空候选、无图、失败、与首商品时间分离及180秒边界，再处理原8位置；不创建／重跑任务，不将该修复前样本混入后续候选bundle矩阵。

I3.1实现后Node语法检查及32类固定断言通过。原始I01-R1首批6候选／6图的回执在155,691ms返回，新增阶段指标在180秒内；首次商品结果仍为453,620ms，最终答复472,629ms，完整回合472,768ms，三个旧阈值均未因此改变。首候选图阶段当前为1／8保留位置、1／6计划执行位置（仅1个已完成观察），不以未执行位置推断系统成功率。三次审批等待缺可核对的完整区间，保持UNKNOWN；宿主工具项目起止包含审批生命周期，与工具duration分开保留。

原报告SHA`0b90315a76acc6e8612026661db814c649fc5517507e6f636703d37658e6321f`及每位置旧计时、调用、失败、版本字段已逐项核对未变。`native-image-performance-pre-fix-scope.json`记录边界：1回合完成但有参数失败／一次纠正，5位置因候选代码审查暂停，2位置准备失败。该I01属于随后URL身份修复前的原生证据，不纳入新bundle最终候选矩阵；尚未创建其他图片任务。
