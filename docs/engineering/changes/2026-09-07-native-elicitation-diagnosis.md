...
## 2026-09-08 approved interactive test follow-up

用户现已授权先发布已完成代码，再切换允许交互审批的测试模式。[v0.17.33 发布记录](../../releases/v0.17.33.md)分别记录 GitHub、Railway 和安装缓存的完成证据。这里没有把用户的模式调整许可转换成网页搜索／Cart 许可。

Research 更新：桌面仍为 `26.901.6511.0`、CLI 仍为 `0.153.4`。[当前官方权限文档](https://learn.chatgpt.com/zh-Hans/docs/permission-modes)说明输入框下方的 Ask for approval／请求审批使用 `workspace-write`、`on-request` 和 `reviewer=user`；Approve for me 不是本次真人表单验收模式。已核对安装 UI 的 `setAgentMode → updateThreadSettingsForNextTurn → thread/settings/update` 调用链，模式对下一轮生效，不能把当前仍在运行的 never 轮次声称已经改好。

当前 Codex App 工具未暴露该 setter，原生桌面控制不可用。本机 app-server 没有显式监听连接；Windows daemon 管理不支持此平台。另启 CLI／App Server 不能冒充连接到当前桌面任务。项目 `.codex/config.toml` 属于默认配置，会被已选 Full Access 预设覆盖；不能靠写该文件声称原生模式已经修复。没有修改全局配置、签名程序、内部数据库或任务状态。

| 可行路线 | 收益 | 范围／风险／代价 | 决策 |
| --- | --- | --- | --- |
| A：用户通过现有任务权限菜单切换 | 最小真实 UI 验证，无插件协议改动 | 同时保存该宿主的模式默认偏好；下一轮生效，测试后可在同菜单恢复旧模式 | 采用；等待用户实际点击，不再次索取同一文本许可 |
| B：定义项目配置，再由用户选 Custom | 可保留明确配置供后续测试 | 仍需 UI 选择，新增持久配置且不能覆盖正在运行的预设；没有省掉当前必要操作 | 本次不新增配置 |

冻结的最小验证步骤：

1. 发布／缓存验证完成后，用户重启 Codex，使用输入框下方权限菜单选择 Ask for approval／请求审批；下一轮核对真实策略。
2. 在明确的新购物目标得到合法 `REQUEST_WEB_SEARCH` 快照后，只调用一次 `begin_web_search`；不伪造 ID，不重置旧目标拒绝状态。
3. 用户确认实际看见表单后自行取消；核对取消／拒绝回执，随后不启动网页读取、不报价、不重试。该测试只验证表单抵达人，不验证商家操作已获许可。
4. 测试后用户可在同一菜单恢复原模式；模式选择会更新该宿主默认偏好，不能声称仅修改一个隔离测试文件。

当前结果：支持的操作入口、作用域与回滚已确认；切换和原生正向测试尚未执行。原因是没有可调用的桌面模式控制接口，不是用户尚未授权。宿主弹窗仍未标为修复。本文为纯文档，无运行构建；只做链接、结构、空白和 diff 检查。
