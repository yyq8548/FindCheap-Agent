...
| 图片与相似项 | 已有两轮复核；本地首批已修复有效视觉差异与旧 EXACT 标签并存，候选和卡片源同步降级 | 统一第 4 节自动转入条件和两类相似方向、保留缺货同款；部分仍受 `allowAlternatives` 限制，`SIMILAR` 仍会阻止综合 primary |
...
| Watch | 本地首批为新建规则增加默认30天，重复创建／重启不续期，显式截止与旧记录保留；本地到期后停止查来源，默认60分钟不变 | 可靠来源准入、首次通知后真正停止宿主调度、跨进程一致性及真实绑定仍未完整实现；旧记录不自动迁移期限 |
| 补货证据 | 本地首批要求同merchant/source/handle/变体/condition的可靠OUT_OF_STOCK→IN_STOCK，优先同键观察且时间不倒退；未知／失败／聚合库存不伪造前态 | 当前来源覆盖与库存语义仍需真实商品验收；本地转变判定不等于已经通知一次并停止调度 |
| 优惠／报价 | 已有优惠证据及共同价格口径；本地首批取消搜索和Coupon研究仅因ZIP触发的隐式cart，保留独立显式报价工具 | 显式工具调用仍不等于可靠用户授权；不预留库存资格及报价／到手价Watch权限需独立核验 |
