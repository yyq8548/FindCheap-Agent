...
2026-09-09 用户批准跨Feed商品冲突处理：按实际merchant/product身份键发现多个版本时，隔离该冲突键的全部版本，保留其他有效商品与商家；不按最低价选赢家、不混合库存/价格。单源重复仍走原严格校验。健康信息保留输入行、隔离组/行和旧快照时间；缓存写入成功与整体发布成功分开，非暂时错误不进行快速重复下载。实施/生产状态见[Awin刷新记录](../engineering/changes/2026-09-09-awin-refresh-followup.md)。
...
2026-09-09本地修复落实既有规则：Sony裸1000XM系列在统一入口与旧Shopify入口先澄清WH/WF，需求快照保留预算；咖啡规格按已选 `Coffee product form` / `Ground or whole bean` 等受控字段判定；合格摘要不计研究线索。报价回执区分商家未审核、能力未知、目标无效及不支持，确定未执行时不暗示购物车已创建。Woo路由保持每轮总计最多6店，其中无相关元数据的探索最多2店；响应详情保留原安全原因码，能力版本2才返回新增诊断，旧客户端兼容。此为源码与自动回归完成，生产及安装仍为此前版本；[实施与未完成验收](../engineering/changes/2026-09-09-plan-remediation.md)单列。
...
3. **编译来源查询**：将中文需求转为来源适用的查询，保留品牌、型号、大类和硬条件；不要把自然语言对话原样当成所有来源的同一查询。
   v0.18.7对已知平台的商品URL按来源分流：已知Shopify链接用于原商品身份和证据读取，WooCommerce接收编译商品文本；私有Woo锚点仍优先，已知Woo链接保留直读，未知URL保留既有注册表查验。来源后端的安全拒绝和商家探索预算不变。[修复与验证](../engineering/changes/2026-09-09-woo-known-shopify-url-followup.md)。
   2026-09-09用户批准的品类优先规则：Woo商家规划单独读取明确productType，口味、颜色或查询同名词不能改变该品类。受控语义能够证明兼容时优先；仅全部已知商家类目均明确跨域冲突时排除。多品类含兼容项保留，同域相邻细类、宽泛／缺失／未支持标签保持UNKNOWN，仍受每轮最多2家探索约束；未知或歧义请求品类沿用原有限路由。已由注册表约束的直接商品URL继续原直读校验。因明确类目冲突导致没有可规划商家时，只能表示本次受限查询完成，不能宣称全部注册商家已查或发无进展续页。视觉入口以已有顶层productType优先，否则转发已验证视觉输入的productType给Woo；不反写持久硬条件。[品类实施记录](../engineering/changes/2026-09-09-woo-category-priority-followup.md)；[视觉传参记录](../engineering/changes/2026-09-09-woo-visual-routing-type.md)。此批准不替代尚未决定的NF07零相关探索提案。
...
5. **核验与补搜**：校验商品大类、身份、变体、用户要求、商家和证据。不足时执行有预算的互补查询或官网查询；必要且符合恢复条件时请求宿主授权的公开网页补搜。
   v0.18.7补明检查后的事实表述边界：成功仅核验本次返回字段；未返回的容量、重量、数量等规格保持未知，不用记忆或用户期望补成事实。可选规格缺口不新增调用、澄清或商品阻断，原有必需规格门槛保留。[说明与验证](../engineering/changes/2026-09-09-returned-spec-evidence-guidance.md)。
...
已知 Shopify 官网商品 URL 只在实际读取并核对原商品后建立服务端私有身份锚点。搜索、恢复与派生快照保留原商家商品路径及明确选定的 variant；同店其他商品、套装或变体不能凭标题重合转正。Shopify 自有变体 ID 与其 URL 选择矛盾时拒绝该候选，不拿 Awin、Woo 或恢复网页的本地 ID 套用此检查。跨店同款沿用共同 GTIN 或品牌加 MPN，并核对已选属性；商家本地 ID 不跨店比较，明确 MPN 冲突不能借共享 GTIN 转正。

CONTINUE 保留 URL 身份，CORRECT／NEW 释放旧商品锚点，其他硬条件按原目标合并规则处理。只有用户明确允许时，非同款才可保留为 `SIMILAR`／`ALTERNATIVE`／`NEEDS_VERIFICATION` 研究候选；检查替代卡不把它改成原目标。明确撤回替代要求后恢复原同款范围，旧快照保持不变。

对已由全球标识证据确认的跨店同款，用户显式换规格时，以实际选中商家和父商品为检查对象；只有来源返回唯一、与请求选项一致的变体，才可更新目标身份及已改规格。未请求的相邻变体、其他商品或其他商家响应不能替换目标；预算及未改硬条件继续保留。实现与独立回归见 [URL 身份记录](../engineering/changes/2026-09-09-shopify-url-identity-followup.md)，不视为最终安装版本的原生验收。
...
v0.18.6 候选将这份版本证据同时用于必选条件、排除条件和偏好，避免原版商品因 FAQ 推荐另一个版本而误判。纯胶囊品类加明确单一机器兼容要求可以编译为品类查询；额外商品品牌、型号、SKU 和复合同款身份不能随之删除。原需求及兼容性核验保持独立，查询扩大不等于要求已满足。源码与原生验证分别记录在[查询修复](../engineering/changes/2026-09-09-query-compatibility-followup.md)及[版本证据修复](../engineering/changes/2026-09-09-host-acceptance-followup.md)。
...
- **重复货况维度**：来源选项中的 Condition、Item Condition、Product Condition 只有与已知 canonical condition 完全一致时才从规格比较键省略。矛盾、未知或含糊值仍禁止同款／单位价格比较；颜色、包装及其他规格不变，商家信任不随可比性提升。
...
Shopify／Woo 商品身份锚点只允许由已验证来源建立并写入私有快照，公共模型输入不得提供。Woo 持久格式复用有界来源字段，核对商家 host、商品／变体关系、URL 所选 ID 与属性，拒绝额外字段和畸形值；无锚点旧快照保持兼容，不从标题补造已确认身份。同一可信任务可恢复原快照，跨任务拒绝；恢复不刷新价格、库存、短期报价或授权。双 MCP 服务实例的保存／重建合同通过，不替代原生桌面重启、UI 选择或宿主生命周期验收。
...
v0.18.4 已在本机 Codex 0.153.4 实际入站验证可信 `_meta.threadId`、重启稳定及另一任务隔离。使用官方 app-server 元数据查询确认归档，不能凭 not-loaded 推断删除。SQLite 保存30日最少历史及原引用，CAS和清理墓碑阻止迟到恢复；旧网页授权不恢复。实际协议、合成来源、真实 stdio 与原生购物验收分开。见[实施证据](../engineering/changes/2026-09-09-complete-remediation-release.md)。

2026-09-09 用户确认：自动删除清理保留设计目标，权威跨连接删除事件缺口标为宿主依赖。实际隔离双连接探针仅创建连接收到删除事件；观察连接在删除前后均可能 not-loaded，不能以此推断删除。插件可控的任务隔离、归档暂停、显式清理和迟到写防护继续保留；完整桌面重启验收与协议验证分别记录。
...
2026-09-09 本地补充：一次性补货的原消息、观察、事件 ID 和生成时间与 COMPLETED 同次原子保存。保存后丢失回执或进程退出时，后续检查可恢复同一历史事件，保持 DELIVERY_UNCONFIRMED，不再次 TRIGGERED、不把历史库存当作当前事实。没有提供模型可调用的送达确认；真实幂等交付和停止调度仍未验收，持续型 Watch 的交付协议未完成。状态格式与旧读者回滚限制见[实施记录](../engineering/changes/2026-09-09-watch-completion-recovery.md)。

2026-09-09 用户确认：真实通知送达确认保留设计目标，当前缺少宿主确认接口的部分标为宿主依赖。Automation 配置回读、PAUSED 状态、本地 COMPLETED 或模型确认均不能替代送达回执；不得新增虚假的确认入口来核销验收。实证见[宿主接口记录](../engineering/changes/2026-09-09-host-acceptance-followup.md)。
...
| 比较记忆 | v0.18.4 基于可信任务的 SQLite/CAS 持久历史及作用域容器；原需求/引用/选择/比较恢复，清理防复活；真实 bundle 重启与源码比较恢复回归通过 | 历史不刷新价格、短期报价或授权；原生桌面完整交互、可靠删除回执仍独立验收；最多500任务记录含墓碑 |
...
v0.18.4 补充：Woo 名单为1002家，新增两家研究来源，不扩展历史信任批准。已确认归档暂停当前任务 Watch；UNKNOWN 不允许启动/执行新任务 Watch。旧无归属记录保留、未自动迁移或停止其宿主调度。补货原通知事件可恢复但无送达/停止 ACK；详见 [v0.18.4 发布边界](../releases/v0.18.4.md)。

v0.18.6 补充：Woo访问表1003家，Hidden Secrets Wigs提供标准变体价格的技术搜索资格；GR-Research类目及Orleans canonical配置修正不增加信任。Awin只有规范化字段冲突的跨Feed商品整键隔离，完全一致的跨Feed行去重保留一份，单Feed重复仍拒绝；输入、发布、隔离、去重分母守恒。刷新终态事件记录状态、重试是否实际安排与失败计数，不能以manifest中途写入或health200声称本轮已完整刷新。当前本地门禁与原生／交付边界见[v0.18.6](../releases/v0.18.6.md)。

同版本后续已实现 Shopify URL 身份保留、显式替代研究卡、已确认跨店同款的显式变体更新，以及严格私有 Woo 快照恢复。公共接口回归及独立审查通过；原生套装错误、独立公共接口复现的 ID／URL 矛盾均保留首次失败证据，最终安装包的原生复验仍待完成。适用规则见第 4.1、8.2 节；完整红绿与验证边界见 [本轮实施记录](../engineering/changes/2026-09-09-shopify-url-identity-followup.md)。
