...
4. **受限并行召回**：调用符合能力与市场范围的 Awin、Shopify、已配置 eBay 和可识别官网；官网可以提前参与，不强制串行“先官网再 Awin”。已知链接优先用于建立身份与商品证据，读取仍过安全边界。
...
| 第二层：可信商家匹配 | 满足当前任务的匹配规则、来自可信商家；相似项另标身份与差异 | 隐藏，不放无关商品补数 |
...
建立前锁定商品、商家范围、颜色／尺码及可靠库存来源。没有可靠来源时说明暂不支持，不建立空监控。先持久化规则并绑定一个宿主 Automation，只有绑定成功才称“正在监控”；失败应撤销新建的孤立调度，不能重复创建。
...
| Watch | 第 9 节明确同意及成功调度绑定 | 不自动购买；归档／删除按第 8 节处理 |
...
下表是本地源码核对，不是新的线上健康证明。历史测试证据见 [v0.17.22 发布记录](../releases/v0.17.22.md)，本轮未重跑业务测试。

| 领域 | 已核对现状 | 为达到本文目标还需做什么 |
| --- | --- | --- |
| 执行层／Backend | 统一执行边界、既有 ports、文本可见引用回执已存在 | 新增持久作用域和预算策略仍要经过同一边界 |
| 检索与官网 | 并行首轮、受限互补与官网补搜已存在；并非严格按展示组串行 | 核验完整用户目标轨迹，不以单来源首屏证明无商品 |
| 图片与相似项 | 已有两轮复核及同款可能／高度相似／同风格判定；部分受 `allowAlternatives` 限制 | 统一第 4 节转入条件和两类相似方向，独立相似推荐不冒充同款；当前 `SIMILAR` 会阻止综合 primary |
| 卡片／Awin 信任 | 三层、避免重复、价格／优惠价值门槛和人工验证 Awin 来源信任路径已存在 | 核验全部新规则；Awin 尚无逐商家审核时间／审核者字段，来源集合需可追溯 |
| 时间 | `SearchRun` 默认 30 秒活跃网络预算，工具单次超时另为 30 秒；网页租期和请求计数另受限 | 90／180 秒完整主动流程预算、阶段输出和 P95 待实现／验收 |
| 比较记忆 | 商品、选择、比较使用进程内 Map；商品快照有短期 TTL | 任务隔离持久化、重启恢复、迁移与主动清理尚未实现 |
| 归档／删除 | 未验证宿主事件及稳定任务身份接入 | 完整接入、调度联动、幂等清理及真实宿主验收 |
| Watch | 本地持久规则、默认 60 分钟及 Automation 绑定存在 | 默认 30 天、首次通知后停止及停止宿主调度未完整实现；期限当前可选 |
| 补货证据 | 当前前态判断为“存在且非 `IN_STOCK`”，可能接受 `UNKNOWN` | 收紧为已确认缺货；加入未知／失败不触发回归 |
| 优惠／报价 | 已有优惠证据、共同价格口径及匿名购物车报价适配器 | 新目标的显式授权与不预留库存资格需独立核验，不能从有接口推断全满足 |
| 业务验收 | 有离线回归与有限真实来源诊断记录 | 独立真实案例、完整桌面授权与重启／生命周期／Watch 轨迹仍待验收 |

## 12. 实施里程碑

以下是后续开发分解，不表示本轮已获执行、提交或部署授权。每个里程碑再按研发协议输出事实底稿、至少两案比较和具备依赖关系的原子计划；以现有模块增量修改，不并行重写全部系统。
...
| 信任与三层排序 | 已核验 Awin 按可信处理，未审核来源不自动转正；无重复占位，无佣金加分，空层隐藏 |
