...
2. 在来源服务配置 `WOOCOMMERCE_SOURCE_ENABLED=true`。默认访问表现为 [200 家商家](woocommerce-merchants-200.md)，表版本 `2026-09-08-expanded-200`。可用经核验的完整 `WOOCOMMERCE_REGISTRY_JSON` 替换；省略使用内置表，不要设置为空字符串。配置变更需重启服务以重建缓存及访问策略。服务端名单扩展由现有 v0.18.0 客户端直接读取，不需要再换插件缓存。
...
商家路由先匹配明确品牌，再匹配品类和常用中英文别名；同分按查询稳定分散。先排除当前冷却或隔离的商家，再填满最多六个位置。`INVALID_RESPONSE` 仅冷却该店相同搜索请求五分钟（最多 2,000 项），不隔离其他关键词或精确读取。429 遵守 Retry-After，403／安全失败保持进程内隔离，需运营核查后重载；旧并发成功不能清掉新隔离。第二页失败保留已读有效商品并标 PARTIAL，原失败同步健康状态，发送后续请求前重新检查。
