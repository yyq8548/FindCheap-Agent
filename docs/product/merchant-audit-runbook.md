...
The current configured adapter gate supports only `feed`, `jsonld`, and `http` sources. Catalog or config claims for `api` or `crawl4ai` are reported as `UNSUPPORTED_SOURCE_TYPE` until their separately hardened adapter path is wired into this gate; this does not grant approval.
