...
当前默认访问表为 [50 家商家](../product/woocommerce-merchants-50.md)，包含原有五家和新增 45 家；该页附新增品牌测试词。每次 Woo 聚合最多选择 6 家，Backend 最多两次聚合。实际生产可用范围还取决于发布配置和各店当时的响应。WooCommerce 是独立搜索来源，但底层为这些商家的单店 API；不是全部 Woo 商家的全球目录。不要预设每次必须出现 Woo 卡片，也不要把名单总数当作本次搜索家数。已安装 v0.18.0 的客户端无需为服务端名单扩展重装。
