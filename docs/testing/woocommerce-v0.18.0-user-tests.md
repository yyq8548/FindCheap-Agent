...
先重启 Codex，再**新建一个任务**，让任务加载更新后的 FindCheap 插件；旧任务可能仍持有旧 MCP 进程。确认插件已启用，安装版本为 `0.18.0` 或对应的 `0.18.0+codex.…` 安装包。下面是发布后的测试步骤，不是已经完成的真实宿主验收。

当前默认访问表为 [200 家商家](../product/woocommerce-merchants-200.md)，包含原有 50 家和本次新增 150 家；该页附新增品牌测试词。每次 Woo 聚合最多选择 6 家，Backend 最多两次聚合。实际生产可用范围还取决于发布配置和各店当时的响应。WooCommerce 是独立搜索来源，但底层为这些商家的单店 API；不是全部 Woo 商家的全球目录。不要预设每次必须出现 Woo 卡片，也不要把名单总数当作本次搜索家数。已安装 v0.18.0 的客户端无需为服务端名单扩展重装。
...
续问：`仍然只要原链接那件 Firm，把预算降到 40 美元，不接受替代品。` 若当前商品价仍为 $48，正确结果是没有符合预算的商品；不能换成更便宜的刷子、床垫或其他精华。恢复预算后仍应绑定原商品。
