    ...
    async upsert(input) {
      await db.query(
        `INSERT INTO products (
          id, brand, manufacturer_part_number, gtins, title, category_path, attributes,
          variant_dimensions, identity_brand_key, identity_mpn_key
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          identity_brand_key = EXCLUDED.identity_brand_key,
          identity_mpn_key = EXCLUDED.identity_mpn_key,
          updated_at = now()`,
        [
          input.productId,
          input.brand,
          ...
          normalizeToken(input.brand),
          input.manufacturerPartNumber === undefined
            ? null
            : normalizeToken(input.manufacturerPartNumber)
        ]
      );
    }
  };
