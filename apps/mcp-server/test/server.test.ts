...
describe("shopping MCP server", () => {
  ...
  it("binds product cards directly to Shopify search and hides legacy rendering from the model", async () => {
    ...
    expect(searchTool?._meta).toMatchObject({
      ui: { resourceUri: "ui://findcheap/product-cards/v42.html" },
      "openai/outputTemplate": "ui://findcheap/product-cards/v42.html"
    });
    expect(renderTool?._meta).toMatchObject({
      ui: {
        resourceUri: "ui://findcheap/product-cards/v42.html",
        visibility: ["app"]
      }
    });
    ...
    expect(resources.resources).toEqual(expect.arrayContaining([expect.objectContaining({
      ...
      uri: "ui://findcheap/product-cards/v42.html",
      mimeType: "text/html;profile=mcp-app"
    }), expect.objectContaining({
      name: "findcheap-product-comparison",
      uri: "ui://findcheap/product-comparison/v12.html",
      mimeType: "text/html;profile=mcp-app"
    })]));

    const resource = await client.readResource({ uri: "ui://findcheap/product-cards/v42.html" });
  ...
  it("exposes only tools backed by configured capabilities", async () => {
    ...
    expect(unifiedTool?.description).toContain("add no optional progress narration");
    expect(unifiedTool?.description).toContain("show only the host Search products label and current requirements");
    expect(unifiedTool?.description).toContain("keep relevant limitations on cards");
    expect(unifiedTool?.description).toContain("Follow required host instructions; skip optional");
    ...
    expect(unifiedTool?.description).toContain("Only RESEARCH_ONLY entries are research leads");
    expect(unifiedTool?.description).toContain("MATCHES_AVAILABLE means matching choices without a primary");
