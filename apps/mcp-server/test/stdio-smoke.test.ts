  ...
  it("uses .mcp.json to initialize the selected plugin bundle with protocol-clean stdout", async () => {
    ...
    try {
      ...
      expect(quotedComparisonTool?._meta).toMatchObject({
        ui: { resourceUri: "ui://findcheap/product-comparison/v12.html" },
        "openai/outputTemplate": "ui://findcheap/product-comparison/v12.html"
      });
      ...
      expect(renderComparisonTool?._meta).toMatchObject({
        ui: {
          resourceUri: "ui://findcheap/product-comparison/v12.html",
          visibility: ["app"]
        }
      });
      ...
      expect(comparisonHtml).not.toContain("暂无已知限制");
      expect(comparisonHtml).toContain('label: text(locale, "Deals", "优惠")');
