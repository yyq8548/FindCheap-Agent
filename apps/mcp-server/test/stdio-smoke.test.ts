  ...
  it("uses .mcp.json to initialize the selected plugin bundle with protocol-clean stdout", async () => {
    ...
    try {
      ...
      expect(quotedComparisonTool?._meta).toMatchObject({
        ui: { resourceUri: "ui://findcheap/product-comparison/v13.html" },
        "openai/outputTemplate": "ui://findcheap/product-comparison/v13.html"
      });
      ...
      expect(renderComparisonTool?._meta).toMatchObject({
        ui: {
          resourceUri: "ui://findcheap/product-comparison/v13.html",
          visibility: ["app"]
        }
      });
